...
void LCD_drawImage(uint16_t usC, uint16_t usP, int width, int height, const char *image){
uint16_t color;
for(int row = 0; row<height; row++){
	for(int column = 0; column<width; column++){
		for(int count = 0; count<2; count++){
		color = (color << 8) + image[80*row+2*column+count];
		}
	LCD_DrawDot(usC+column, usP+row, color);
	color = 0x0;
	}
}
}
